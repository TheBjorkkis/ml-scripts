...
# The code doesn't run because of the indice problem. I tried at least.
##-your-code-starts-here-##
for indice in pairs[:,0]:
    row = distmat_sorted[:, int(indice)]
    val = np.abs(row[0]-row[1])/np.abs(row[0]-row[2])
    nndr[int(indice)] = val
    
nndr = np.sort(nndr, axis=0)
