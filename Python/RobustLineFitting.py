...
while N > sample_count:
    ...
    # Form the line by cross product and normalizing it.
    l = np.cross(sample1,sample2)
    l = l/np.sqrt(a^2+b^2)
    ##-your-code-ends-here-##
    ...
    # Go through each data point and calculate if those are close enough the line.
    # If its close enough, add the index to the inliers.
    for data_point in range(int(m)):
        if np.dot(l, points_h[:,data_point]) < t:
            inliers.append(data_point)
    ##-your-code-ends-here-##
...
# Check how many inliers are located near the formed best line.
for data_point in range(int(m)):
    if np.dot(best_line, points_h[:,data_point]) < t:
        x_inliers.append(points_h[:,data_point][0])
        y_inliers.append(points_h[:,data_point][1])
