...
# Compute the NCC of pixels' intensities
# for all pairs of patches extracted from the two images
distmat = np.zeros((npts1, npts2))
for i1 in range(npts1):
    for i2 in range(npts2):
        f = patches1[:,:,i1]
        g = patches2[:,:,i2]
        upper_part = np.sum( (g-np.mean(g))* (f-np.mean(f)) )
        lower_part = np.sqrt( np.sum( (g-np.mean(g))**2 )* np.sum( (f-np.mean(f))**2 ) )
        distmat[i1, i2] = upper_part/lower_part
